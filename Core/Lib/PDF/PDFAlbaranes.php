 ...
 */
abstract class PDFAlbaranes extends PDFCore
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->lineHeaders = [
            'albaran' => ['type' => 'text', 'title' => $this->i18n->trans('delivery-note')],
            'total' => ['type' => 'number', 'title' => $this->i18n->trans('total')]
        ];
    }
    ...
    protected function insertBusinessDocBody($model)
    {
        ...
        foreach ($arrayAlbaranes as $line) {
            ...
            $arrayAlbaranesWhere = $albaran->all($where);
            $data = [];
            
            foreach ($arrayAlbaranesWhere as $linea) {
