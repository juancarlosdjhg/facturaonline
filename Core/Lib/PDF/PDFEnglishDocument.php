 ...
 */
abstract class PDFEnglishDocument extends PDFCore
{
    ...
    public function __construct()
    {
        parent::__construct();
        $this->lineHeaders = [
            'referencia' => ['type' => 'text', 'title' => $this->i18n->trans('reference-english') . ' - ' . $this->i18n->trans('english-description')],
            'cantidad' => ['type' => 'number', 'title' => $this->i18n->trans('quantity-english')],
            'pvpunitario' => ['type' => 'number', 'title' => $this->i18n->trans('price-english')],
            'dtopor' => ['type' => 'percentage', 'title' => $this->i18n->trans('dto-english')],
            'dtopor2' => ['type' => 'percentage', 'title' => $this->i18n->trans('dto-2-english')],
            'pvptotal' => ['type' => 'number', 'title' => $this->i18n->trans('net-english')],
            'iva' => ['type' => 'percentage', 'title' => $this->i18n->trans('tax-english')],
    ...
    protected function combineAddress($model): string
    {
        ...
        $completeAddress .= empty($model->apartado) ? '' : ', ' . 'Box' . ' ' . $model->apartado;
    ...
    protected function insertBusinessDocBody($model)
    {
        ...
        foreach ($model->getlines() as $line) {
            ...
            foreach ($this->lineHeaders as $key => $value) {
                if ($key === 'referencia') {
                    $data[$key] = empty($line->{$key}) ? Utils::fixHtml($line->description) : Utils::fixHtml($line->{$key} . " - " . $line->description);
    ...
    protected function insertBusinessDocFooter($model)
    {
        ...
        if (count($taxRows) > 1) {
            ...
            $this->pdf->ezText("\n");
            $this->pdf->ezTable($taxRows, $taxHeaders, 'Taxes' , $taxTableOptions);
        }
    ...
    protected function insertBusinessDocHeader($model)
    {
        $headerData = [
            ...
            'subject' => $this->i18n->trans('customer-english'),
            'fieldName' => 'nombrecliente'
        ];

        if (isset($model->codproveedor)) {
            $headerData['subject'] = $this->i18n->trans('supplier-english');
        ...
        if (!empty($model->direccion)) {
            $tableData[] = ['key' => 'Address', 'value' => $this->combineAddress($model)];
        }

        /// rectified invoice?
        if (isset($model->codigorect) && !empty($model->codigorect)) {
            array_unshift($tableData, ['key' => 'Original', 'value' => $model->codigorect]);
        }
    ...
    protected function insertBusinessDocShipping($model)
    {
        ...
        if ($contacto->loadFromCode($model->idcontactoenv)) {
            ...
            $tableData = [
                ['key' => 'Name', 'value' => $name],
                ['key' => 'Address', 'value' => $this->combineAddress($contacto)],
            ];
    ...
    {
        $now = $this->i18n->trans('generated-at-english', ['%when%' => date('d-m-Y H:i')]);
        ...
        if (count($receipts) > 0) {
            ...
            foreach ($receipts as $receipt) {
                ...
                $rows[] = [
                    ...
                    'vencimiento' => $receipt->pagado ? 'Paid' : $receipt->vencimiento
                ];
            }
            $tableOptions = [
                'cols' => [
